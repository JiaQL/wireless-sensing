...
Existing material identification systems such as Radar, X-Ray, CT, MRI and B-scan ultrasonography use special hardware with high frequency, large bandwidth and antenna arrays, which are extremely expensive and usually large in size. Recently, some works utilize the wireless signal to sense the target material. For examples, \cite{Tagscan} use the property of wireless signals propagating in liquids to identify the different material, which is the changes of RSS and phase. Similarly, \cite{LiquID} utilizes cheap UWB devices with 1GHz bandwidth to extract the dielectric constant for fine-grained liquid sensing.
